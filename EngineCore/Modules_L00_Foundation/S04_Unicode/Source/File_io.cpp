...
UnicodeString LoadWholeFile(const std::filesystem::path& aPath, std::optional<Charset> aCharset) {
    ...
    if (aCharset.has_value()) {
        ...
        const auto detectionResult =
            CharsetDetector(contents.c_str(), static_cast<std::int32_t>(contents.size())).detect();
    ...
    default:
        HG_THROW_TRACED(TracedRuntimeError,
                        0,
                        "Could not detect charset of file at '{}'.",
                        aPath.string());
    }

    return UnicodeString{contents.data(), static_cast<std::int32_t>(contents.size()), codepage};
}
