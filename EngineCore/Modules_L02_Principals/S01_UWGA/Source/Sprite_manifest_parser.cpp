...
const auto TEXTURE_REGEX_PATTERN = HG_UNISTR(
    R"_(\s*@texture\(\s*([0-9xABCDEF]+)\s*,\s*([0-9xABCDEF]+)\s*,\s*([_a-zA-Z]+)\s*\)\s*:\s*)_");

TexturePackingHeuristic StringToTexturePackingHeuristic(const std::string_view& aString) {
    using namespace std::literals;
    if (aString == "BEST_SHORT_SIDE_FIT"sv) {
        return TexturePackingHeuristic::BEST_SHORT_SIDE_FIT;
    } else if (aString == "BEST_LONG_SIDE_FIT"sv) {
        return TexturePackingHeuristic::BEST_LONG_SIDE_FIT;
    } else if (aString == "BEST_AREA_FIT"sv) {
        return TexturePackingHeuristic::BEST_AREA_FIT;
    } else if (aString == "BOTTOM_LEFT_RULE"sv) {
        return TexturePackingHeuristic::BOTTOM_LEFT_RULE;
    } else if (aString == "CONTACT_POINT_RULE"sv) {
        return TexturePackingHeuristic::CONTACT_POINT_RULE;
    }
