...
void TopDownLineOfSightRenderer::render() {
    _renderTexture.clear(hg::uwga::COLOR_WHITE);
...
#if 0
    {
        hg::uwga::CircleShape circle{4.f};
...
std::optional<bool> TopDownLineOfSightRenderer::testVisibilityAt(PositionInWorld aPos) const {
    ...
    return (hg::uwga::Color{p[0], p[1], p[2], 255} != hg::uwga::COLOR_BLACK);
}

const hg::uwga::Texture& TopDownLineOfSightRenderer::__ggimpl_getTexture(
...
void TopDownLineOfSightRenderer::_renderOcclusion() {
    ...
    hg::uwga::Vertex vertices[10];
    for (auto& vertex : vertices) {
        vertex.color = hg::uwga::COLOR_BLACK;
    }
    ...
    for (hg::PZInteger y = startGridY; y <= endGridY; y += 1) {
        for (hg::PZInteger x = startGridX; x <= endGridX; x += 1) {
            ...
            if (const auto* cell = _world.getCellAtUnchecked(x, y);
                ...
                _renderTexture.draw(vertices, 10, hg::uwga::PrimitiveType::TRIANGLE_STRIP);
            }
            // ****************************************************
        }
    }
